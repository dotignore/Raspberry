...
using Poco::Net::ServerSocket;                  // set-up a server socket
using Poco::Net::HTTPRequestHandler;
using Poco::Net::HTTPRequestHandlerFactory;
using Poco::Net::HTTPServer;                    // set-up a HTTPServer instance
...
public:
    ...
    void handleRequest(HTTPServerRequest& request, HTTPServerResponse& response)
    {
        ...
        ostr << "<!--  -------------- Request Form Answer-------------- -->\n";
        ostr << "<h2>Request111</h2><p>\n";
        ostr << "Method: " << request.getMethod() << "<br>\n";              // Method: GET  Method: POST
        ostr << "URI: " << request.getURI() << "<br>\n";                    // URI: /form?text=321 URI: /form
        ...
        if (!form.empty())
        {
            ostr << "<!--  -------------- GET/POST Answer -------------- -->\n";
            ostr << "<h2>Form111</h2><p>\n";
            it = form.begin();
            end = form.end();
            for (; it != end; ++it)
            {
...
protected:
    ...
    int main(const std::vector<std::string>& args)
    {
        ...
        {
            unsigned short port = (unsigned short) config().getInt("HTTPFormServer.port", 8080);
